...
export default class Camera_handle{
    ...
    public poses:Object = {"NOSE":[-1,-1],"L_SHOULDER":[-1,-1],"R_SHOULDER":[-1,-1]};
    ...
    constructor() {
        ...
        if (!this.videoElement)this.videoElement = document.getElementById('video') as HTMLVideoElement
        if (!this.canvasElement) this.canvasElement = document.getElementById('canvas') as HTMLCanvasElement;
    ...
    private onResults(results) {
      ...
      if(!results.segmentationMask){console.log("could not detect your body");return;}
